...
use SMConfig\Exception\SMInvalidPathException;
use SMConfig\Exception\SMInvalidFileExtException;
use SMConfig\Exception\SMInvalidFileContentException;
use SMConfig\Exception\SMInvalidConfigNameException;
...
class SMConfig
{
    ...
    public static function getInstance($files = [])
    {
        if (is_null(self::$sInstance)) {
            self::$sInstance = new SMConfig($files);
        }
    ...
    private function __construct(array $configFiles = [])
    {
        ...
        if (!empty($this->configFiles)) {
            $this->validateConfigFiles($this->configFiles);
            $this->setConfigwithFiles($this->configFiles);
        }
    }
     ...
     * we reset the config from the files stored in th SMConfig object
     *
     * @return void
     */
     ...
     * ]
     *
     * @throws \SMConfig\Exception\SMInvalidConfigNameException
     * @throws \SMConfig\Exception\SMInvalidPathException
     * @throws \SMConfig\Exception\SMInvalideFileExtException
     *
     * @return void
     */
    private function validateConfigFiles(array $configFiles)
    {
        ...
        foreach ($configFiles as $configName => $filePath) {
            ...
            if (!is_string($configName)) {
                throw new SMInvalidConfigNameException(
            ...
            if (!file_exists($filePath)) {
                throw new SMInvalidPathException("The file '$filePath' doesn't exist");
            }

            $ext = \explode(".", $filePath);
            $ext = end($ext);
            if (!\in_array($ext, self::$sAllowedExt)) {
                throw new SMInvalidFileExtException(
     ...
     * ]
     *
     * @throws \SMConfig\Exception\SMInvalideFileContentException
     *
     * @return void
     */
    private function setConfigFiles(array $configFiles)
    {
        foreach ($configFiles as $configName => $filePath) {
            ...
            if (!\is_array($configContent)) {
                throw new SMInvalidFileContentException("The content of the config file is not an array!");
            }
